...
- **One platform.** Built and tested on Windows 11, MSVC 19.42, x64. The `memcpy` conversion is
  portable in principle; it has not been exercised elsewhere, and a portability claim resting on one
  compiler is not a portability claim.
- **Fast maths does not break it, on the one compiler tried.** The default build is `/fp:strict`,
  but the whole suite passes under `/fp:fast` as well, which is checked rather than assumed:
  `cmake -S . -B build-fast -DSTATE_CHANNEL_FAST_MATH=ON`. The reason it survives is that every
  guard classifies the integer bits and never compares the float, so there is no comparison for the
  optimiser to reason about. Note that on GCC and Clang `-ffast-math` additionally sets
  flush-to-zero and denormals-are-zero at startup. That cannot change what these guards decide,
  because `memcpy` sees the bit pattern either way, but it can change what the host hands you.
